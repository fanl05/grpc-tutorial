 ...
 * @author Ryland
 */
public class SWHForecastServer extends SWHForecastServiceGrpc.SWHForecastServiceImplBase {

    public static void main(String[] args) {
        try {
            ServerBuilder.forPort(9999).addService(new SWHForecastServer()).build().start();
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
